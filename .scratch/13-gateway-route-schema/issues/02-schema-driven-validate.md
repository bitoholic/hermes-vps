...
- **Today's exact `validate.yml` checks, for reference against ticket #01's schema** (confirmed by reading the current file): `host`/`upstream`/`mfa` are checked for definedness, `upstream` additionally for non-emptiness, `mfa` additionally for being boolean, `tls_mode` (if defined) for being in `["internal", "auto"]`, `port` (if defined) for being an integer, and `basic_auth_user`/`basic_auth_hash` for mutual presence only. Nothing today type-checks `host`, and nothing today type-checks `basic_auth_user`/`basic_auth_hash` individually. Ticket #01's schema adds `string` constraints on those three — a deliberate, low-risk narrow expansion, not a pure transcription. Implement the schema-driven engine against ticket #01's schema as written (including those three `string` checks); just don't describe the result as behavior-identical to today.
...
- Open question from the spec's Further Notes, left to the implementer: whether a route with multiple simultaneous field violations should report all of them in one message or fail on the first. Existing tests only assert pass/fail, not exact message content, so either is acceptable — pick whichever is simpler to write correctly in Jinja and note the choice here once decided.
  - **Decided**: fail-on-first. `validate_route.yml`'s per-field `assert` is looped over `gateway_route_schema` with Ansible's default loop behavior — the first schema field a route violates produces the failure message; later fields in the loop for that same route aren't evaluated once the play halts. Simpler to get right in Jinja than accumulating and joining every violation across every field, and the ticket's own acceptance criteria only ever asked for a message naming *a* violated field, not an exhaustive list.
