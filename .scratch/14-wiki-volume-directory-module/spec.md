...
- **`meta/main.yml` dependency declarations**: every migrated role (`authelia`, `backup`, `conduit`, `docker`, `hermes`, `owntracks`, `silverbullet`) already declares `- role: wiki_volume` as a dependency — verified directly, no role needs a new dependency added for this epic. This ticket only changes what each role's tasks do, not its dependency graph.
